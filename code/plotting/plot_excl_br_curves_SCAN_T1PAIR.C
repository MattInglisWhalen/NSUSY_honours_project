...
void plot_excl_br_curves_SCAN_T1PAIR(char* stordir,char* ansystag,char* ztag=""){

  cout << "Processing " << ansystag << endl;

  get_search_data(ansystag);

  read_data(stordir,ansystag,ztag,0,STOP1,NEUT1,GLUINO);
  read_data(stordir,ansystag,ztag,1,STOP1,NEUT1,GLUINO);
  read_data(stordir,ansystag,ztag,2,STOP1,NEUT1,GLUINO);
...
// ------------------------------------------------ making plots ---------------------------------------------------- //
  ...
  string* canvname=new string("Stop Pair Production, Fractional Branching Ratios");
...
//  canv->SetGridy();
  ...
  string* framename=new string(ansystag);
  framename->append(" 95% CL exclusion for pp#rightarrow#tilde{t}_{1}#bar{#tilde{t}}_{1}");
  string* pdfname=new string(ansystag);
  pdfname->append("_SCAN_T1PAIR.pdf");

  TH1F* frame=new TH1F("frame",framename->c_str(),1000,200,600);
  frame->SetStats(0);
  frame->SetMinimum(0);
  frame->SetMaximum(400);
  frame->GetXaxis()->SetTitle("#tilde{t}_{1}   /   GeV");
  frame->GetXaxis()->SetTickLength(0.02);
  frame->GetXaxis()->SetLabelSize(0.025);
  frame->GetXaxis()->SetLimits(0,1000); //minx/maxx limits
  ...
  kinemat_stop->SetPoint(1,800+2*TOPMASS,800);
  ...
  kinemat_sbot->SetPoint(1,800+2*BOTMASS,800);
  kinemat_sbot->SetLineWidth(2);
  kinemat_sbot->Draw("L");

  TLatex* go_ttxn1 = new TLatex(450,130,"#font[22]{#tilde{t}_{1}#rightarrowt#tilde{#chi}_{1}^{0} forbidden}");
  go_ttxn1->SetTextSize(0.019);
  go_ttxn1->SetTextAngle(58);
  go_ttxn1->Draw();

  TLatex* go_bbxn1 = new TLatex(300,310,"#font[22]{#tilde{t}_{1}#rightarrow b#tilde{#chi}_{1}^{+} forbidden}");
  go_bbxn1->SetTextSize(0.019);
  go_bbxn1->SetTextAngle(58);
  go_bbxn1->Draw();
  ...
  for(Double_t iBR=plot_opts[0];(iBR<=1.)&&(iBR>=0.);iBR=iBR+plot_opts[1]){
    ...
    find_excluded_GPs(iBR,STOP_PAIR_PRODUCTION,T1_TN1_BX1,NODECAY,EXACT);
    ...
    if(PRINT_CONTOUR_TMODE){
      find_excluded_GPs(iBR,STOP_PAIR_PRODUCTION,T1_TN1_BX1,NODECAY,TMODE);
    ...
    if(PRINT_CONTOUR_BMODE){
      find_excluded_GPs(iBR,STOP_PAIR_PRODUCTION,T1_TN1_BX1,NODECAY,BMODE);
    ...
    if(PRINT_CONTOUR_BOTH){
      find_excluded_GPs(iBR,STOP_PAIR_PRODUCTION,T1_TN1_BX1,NODECAY,BOTH);
    ...
    if(PRINT_CONTOUR_GEOM){
      find_excluded_GPs(iBR,STOP_PAIR_PRODUCTION,T1_TN1_BX1,NODECAY,GEOM);
