...
void plot_excl_br_curves_GOMED_T1_MU_BINO(char* stordir,char* ansystag,char* ztag=""){
...
// ------------------------------------------------ making plots ---------------------------------------------------- //
  ...
  string* canvname=new string("GOMED_T1_MU_BINO");
  string* framename=new string("pp#rightarrow#tilde{g}#tilde{g} , #tilde{g}#rightarrow#tilde{t}_{1}#bar{t}");
  framename->append(" , #tilde{#chi}_{1}^{0}~#tilde{B} , #tilde{#chi}_{1}^{#pm}~#tilde{H}^{#pm}");
...
//  canv->SetGridy();
  ...
  pdfname->append(ansystag); pdfname->append("_GOMED_T1_MU_BINO.pdf");
